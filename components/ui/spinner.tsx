...
type SpinnerProps = (LucideProps & { size?: number; className?: string }) & {
  variant?:
    | 'default'
    | 'circle'
    | 'pinwheel'
...
const Infinite = ({ size = 24, ...props }: SpinnerVariantProps) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 100 100" preserveAspectRatio="xMidYMid" {...props}>
    <title>Loading...</title>
    <path
      d="M20,50 C30,20 50,20 60,50 C50,80 30,80 20,50"
      fill="none"
      stroke="currentColor"
      strokeWidth="6"
      strokeLinecap="round"
    ...
    >
      <animate attributeName="stroke-dashoffset" from="0" to="140" dur="2s" repeatCount="indefinite" />
    </path>
  </svg>
);

export default function Spinner({ variant = 'default', size = 24, className, ...props }: SpinnerProps) {
  const common = { size, className } as const;
  switch (variant) {
    case 'circle':
      return <Circle {...common} {...props} />;
    case 'pinwheel':
      return <Pinwheel {...common} {...props} />;
    case 'circleFilled':
      return <CircleFilled {...common} {...props} />;
    case 'ellipsis':
      return <Ellipsis size={size} {...props} />;
    case 'ring':
      return <Ring size={size} {...props} />;
    case 'bars':
      return <Bars size={size} {...props} />;
    case 'infinite':
      return <Infinite size={size} {...props} />;
    case 'default':
    default:
